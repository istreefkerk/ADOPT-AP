...
class GlobalTimeVarPts:
	...
	def extract_point_var_UZ_inf(self,nodes,inf):
		"""extract variables from infiltration component
		"""
	...
	def extract_point_var_UZ_swb(self, nodes, swb, *layer):
		...
		INPUT
		-----
		nodes:	nodes at which variables will be extracted
		swb:	dryp components
	...
	def extract_point_var(self, nodes, var):
		"""extract variables from groundwater component
		second layer
		"""
	...
	def save_point_var(self, fname, time, area, rarea, depth=1, *dt):
		...
		-----
		fname:	file name
		time:	time series of output file
		area:	area of catchment of points
		rarea:	riparian area of selected points
		OUTPUT
		------
		dataset of model outputs in csv format
		"""
		
		# aggregation for one hour
		#point_aggre = None
		if dt:
			if dt[0] < 60:
				point_aggre = 'H'
		...
		if self.Kc:
			df = pd.DataFrame()
			df['Date'] = time
			data = np.transpose(self.Kc)
			
			for i in range(len(data)):			
				field = 'Kc_'+str(i)				
				df[field] = data[i,:]
			
			if print_cropKc is True:
				fname_out = fname+'_Kc.csv'			
				os.remove(fname_out) if os.path.exists(fname_out) else None			
				if point_aggre is not None:
					df.index = pd.DatetimeIndex(df['Date'])
					df = df.resample(point_aggre).mean()
...
class GlobalTimeVarAvg:
	"""Setting variables and arrays for saving model outputs
	"""
	...
	def __init__(self,*factor):
		...
		*factor	: Catchment area factor for fluxes estimation
		...
		self.AET = []
		self.INF = []	# Infiltration rate [mm/day]
		self.ROF = []	# input for surface component
		self.IUZ = []	# Infiltration rate [mm/day]
		self.RUZ = []
		self.LUZ = []
		self.EXS = []	# Runoff [mm/day]
		#self.OFL = []	# Overlandflow [mm/day]
	...
	def extract_avg_var_OF(self, nodes, ro):
		...
		self.ROF.append(np.sum(area_catch_factor*ro.run_dt[nodes]))
	...
	def extract_avg_var_UZ_swb(self, nodes, swb):
		"""extract variables from soil layer component
		"""
		if len(self.cth_factor):		
			area_catch_factor = np.array(self.cth_factor[nodes])		
		else:		
		...
		self.IUZ.append(np.sum(area_catch_factor*swb.inf_dt[nodes]))
		...
		self.RUZ.append(np.sum(area_catch_factor*swb.sro_dt[nodes]))
		self.LUZ.append(np.sum(area_catch_factor*swb.luz_dt[nodes]))
	...
	def save_avg_var(self, fname, time):
		"""save average mode variables for water balance 
		as csv files for selected points
		INPUT
		-----
		fname:	file name
		time:	time series of output file
		...
		OUTPUT
		------
		dataset of average model outputs in csv format
		"""
		df = pd.DataFrame()		
		df['Date'] = time
		...
		# aggregate results to reduce use of space
		if agg_balance_global is not None:
			df_aux = df.copy()
			df_aux.index = pd.DatetimeIndex(df_aux['Date'])
			df_aux = df_aux.resample(agg_balance_local).mean()
			df_aux = df_aux.reset_index()
			
			df.index = pd.DatetimeIndex(df['Date'])
			df = df.resample(agg_balance_local).sum()
			df = df.reset_index()
			
			if self.WTE:
				df['WTE'] = np.array(df_aux['WTE'])
			if self.AET:
				df['THT'] = np.array(df_aux['THT'])
				df['LUZ'] = np.array(df_aux['LUZ'])
...
class GlobalGridVar:
	...
	def __init__(self, env_state, inputfile):
		...
		if self.save_results == 1:
			self.lat = env_state.grid.node_y.reshape(env_state.grid.shape)
			self.lon = env_state.grid.node_x.reshape(env_state.grid.shape)
	...
	def get_env_state(self, date_sim_dt, t_date, pre, pet, inf, swb, ro, gw, swb_rip, env_state):
		...
		if self.save_results == 1:
			...
			self.OFL_dt += ro.dis_dt.reshape(self.grid_shape)
			self.QFL_dt += ro.qfl_dt.reshape(self.grid_shape)
			self.TLS_dt += ro.tls_dt.reshape(self.grid_shape)
			self.FRH_dt += swb_rip.pcl_dt.reshape(self.grid_shape)
			self.DRH_dt += swb.pcl_dt.reshape(self.grid_shape)
			self.GDH_dt += env_state.SZgrid.at_node['discharge'][:].reshape(self.grid_shape)
			self.WTE_dt += gw.wte_dt.reshape(self.grid_shape)		
			self.nsteps += 1
			
			if date >= self.idate:
				# append data into the xarray
				...
				# Save variables and
				# save as netcdf and empty the xarray
	...
	#def	save_netCDF_grid(self, fname):
	#	"""Save variables as grided netcdf files
	#	"""
	#	if self.save_results == 1:
	#		df = xr.Dataset({#"aet": (['time', 'lat', 'lon'], valid_models),
	#			"pre": (['time', 'lat', 'lon'], (self.PRE[j][:]).reshape(self.grid_shape)),
	#			"pet": (['time', 'lat', 'lon'], (self.PET[j][:]).reshape(self.grid_shape)),
	#			"aet": (['time', 'lat', 'lon'], (self.AET[j][:]).reshape(self.grid_shape)),
	#			"tht": (['time', 'lat', 'lon'], (self.THT[j][:]).reshape(self.grid_shape)),
	#			"inf": (['time', 'lat', 'lon'], (self.INF[j][:]).reshape(self.grid_shape)),
	#			"run": (['time', 'lat', 'lon'], (self.OFL[j][:]).reshape(self.grid_shape)),
	#			"qfl": (['time', 'lat', 'lon'], (self.QFL[j][:]).reshape(self.grid_shape)),
	#			"tls": (['time', 'lat', 'lon'], (self.TLS[j][:]).reshape(self.grid_shape)),
	#			"fch": (['time', 'lat', 'lon'], (self.FRH[j][:]).reshape(self.grid_shape)),
	#			"dch": (['time', 'lat', 'lon'], (self.DRH[j][:]).reshape(self.grid_shape)),
	#			"gdh": (['time', 'lat', 'lon'], (self.GDH[j][:]).reshape(self.grid_shape)),
	#			"wte": (['time', 'lat', 'lon'], (self.WTE[j][:]).reshape(self.grid_shape)),
	#			#"aet": (['time', 'lat', 'lon'], (self.AET[j][:]/self.PET[j][:]).reshape(self.grid_shape)),
	#			},
	#			coords={
	#					"lon": ('lon', ds.lon),
	#					"lat": ('lat', ds.lat),
	#					"time": (np.arange(len(weight))),
	#			})
	#		print('save netCDF4')
...
def check_mass_balance(fname, outavg, outOF, outavg_AER, data, dt, area):	
	...
	df['gws'] = np.array(data[4])*1000.0
	df['uzs'] = np.array(data[5])
	df['gbf'] = np.array(data[6])*1000.0
	df['aet'] = np.array(data[7])
	df['egw'] = np.array(data[8])
	df['chs'] = np.append(0,np.diff(np.array(data[9])))
	df['rzs'] = np.array(data[10])
	df['bcsz'] = np.array(data[11])*1000.0
	
	# save data from interception component
	if len(data) > 12:
		df['pth'] = np.array(data[12])
		df['eca'] = np.array(data[13])
		df['lai'] = np.array(data[14])
		df['kc'] = np.array(data[15])	
		df['kcr'] = np.array(data[16])
		df['eacr'] = np.array(data[17])
		df['pthr'] = np.array(data[18])
	
	# aggregate results to reduce use of space
	if agg_balance_global is not None:
		df.index = pd.DatetimeIndex(df['Date'])
		df = df.resample(agg_balance_global).sum()
		df = df.reset_index()
