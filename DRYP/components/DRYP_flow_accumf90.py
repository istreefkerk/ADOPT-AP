...
class runoff_routing(object):
	...
	def run_runoff_one_step(self, exs, swb, aof, env_state, data_in):
		...
		if check_dry_conditions > 0:
			...
			env_state.grid.at_node['runoff'][env_state.grid.closed_boundary_nodes] = 0
			
			# Make the precision of all arrays consistent with Fortran objects
			discharge = np.array(env_state.grid.at_node['runoff']
						*env_state.area_cells, np.float32)#Qw			
						
			QTL = np.zeros_like(discharge, np.float32) #QTL
		...
		self.qfl_dt[act_nodes] = np.array(
				env_state.grid.at_node['Q_ini'][act_nodes]
				/env_state.area_cells[act_nodes])
