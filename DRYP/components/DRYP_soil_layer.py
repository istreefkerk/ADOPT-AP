...
class swbm(object):
	...
	def __init__(self, env_state, Duz, tht_t0, data_in, *layer):
		...
		self.kc_dt = np.zeros(env_state.grid_size)
	...
	def run_swbm_one_step(self, inf, pet, Kc, Ksat, env_state, data_in, *nodes):
		...
		env_state: 	additional variables as python objects
		nodes:		(optional) id-nodes to process
		"""
		if nodes:		
			act_nodes = nodes[0]		
		else:		
		...
		self.inf_dt = np.zeros(len(self.aet_dt))
		self.inf_dt[act_nodes] = inf_dt
		
		self.luz_dt = np.zeros(len(self.aet_dt))
		self.luz_dt[act_nodes] = L - L_0
		...
		#self.pcl_dt += aux_pcl
		#
		## actual evapotranspiration for cells with depth zero
		#aux_aet = np.array(inf - pet)
		#aux_aet[aux_aet > 0] = 0
		#aux_aet = np.array(pet - aux_aet)
	...
	def run_swb_lat_flow_one_step(self, env_state_soil, env_state_rip):
		...
		K_soil = np.array(
			env_state_soil.grid.at_node['Ksat_uz'][river_cell])
		K_riv = np.array(
			env_state_soil.grid.at_node['Ksat_uz'][river_cell])
...
def SWBMh(I, PET, Kc, L0, SMD_0, z_soil, fs, fc, wp, c, Ksat):
	...
	# Calculate evaporation due to precipitation/infiltration
	I_AET = np.where(I > PET, PET, I)
	...
	kd = (1-c)*Ksat/(z_soil*np.power(Lsat, c))
	
	# calculate the water content after drainage
	Lt = np.exp(1/(1-c)*np.log(np.power(L_aux, 1-c) - kd))
	
	# calculate the variation of water content under due to drainage
	D = np.where(Lt > Lfc, L_aux-Lt, L_aux-Lfc)
	
	# check if the change in water content do not fall below field
	# capacity due to drainage
	D[D < 0] = 0
	
	# Update drainage under infiltration excess
	Ks1 = Ksat - D
