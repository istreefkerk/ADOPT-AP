...
class ABMconnector(object):
    def __init__(self, inputfile, env_state):
        ...
        if inputfile:
            ...
            self.actual_transpiration_grass = np.empty((env_state.grid_size, 90)) # half a year  = 201
            self.actual_transpiration_grass[:] = np.nan
            self.potential_transpiration_grass = np.empty((env_state.grid_size,90)) # half a year
